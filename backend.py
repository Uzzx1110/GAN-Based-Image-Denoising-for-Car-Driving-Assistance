...
# === Load Models ===
# Load your generator (CycleGAN generator for enhancing)
...
# Load YOLO models
obj_model = YOLO(r'C:\Users\DELL\Documents\GitHub\AutonomousDrivingGanMV\best.pt')       
sign_model = YOLO(r'C:\Users\DELL\Documents\GitHub\AutonomousDrivingGanMV\runsnano\exp12\weights\best.pt')  

# === Frame Processing Function ===
def process_frame(frame, obj_model, sign_model, enet_model):
    vis_frame = frame.copy()

    # === Lane Detection ===
    input_image = cv2.resize(frame, (512, 256))
    input_image_gray = cv2.cvtColor(input_image, cv2.COLOR_BGR2GRAY)[..., None]
    ...
    binary_seg_resized = cv2.resize(binary_seg, (frame.shape[1], frame.shape[0]), interpolation=cv2.INTER_NEAREST)

    red_mask = np.zeros_like(frame)
    red_mask[binary_seg_resized == 1] = [0, 0, 255]
    ...
    for box in obj_results.boxes:
        ...
        cv2.putText(lane_overlay, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    for box in sign_results.boxes:
        ...
        cv2.putText(lane_overlay, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

    return lane_overlay

# === Video Processing Function ===
def process_video(input_source, output_path, obj_model, sign_model, enet_model, is_webcam=False):
    cap = cv2.VideoCapture(input_source if not is_webcam else 0)
